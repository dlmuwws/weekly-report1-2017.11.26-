...
vector<oper_rotation*>add_string(int*base, vector<vector<string_>>type_strings, double aircraft_dual, vector<leg*>leg_list, int M_num, vector<vector<G_arc*>>daily_gc_, vector<leg*>weekly_leg_list_);
...
pair<bool, int*>SPFA2(vector<seq_node>nodes, int source, int num_, vector<vector<string_>>three_day_strings, int M_num);
...
vector<oper_rotation*>add_string(int*base, vector<vector<string_>>type_strings, double aircraft_dual, vector<leg*>leg_list, int M_num, vector<vector<G_arc*>>daily_gc_, vector<leg*>weekly_leg_list_) // type_strings是各种string,s1-s4
{
	...
	while (!queue_.empty())
	{
		...
		if (u>=M_num+size1&&u<M_num+size1+size2)
		{
			for (int i = 0; i<nodes[u].children.size(); i++)
			{
				...
				if (dis_[nodes[u].children[i].first]>dis_[u] + nodes[u].children[i].second &&can_connect(s2_[pre_[u]-M_num],s4_[nodes[u].children[i].first -M_num-size1-size2]))
				{
					dis_[nodes[u].children[i].first] = dis_[u] + nodes[u].children[i].second;
					pre_[nodes[u].children[i].first] = u;
...
//	int t = 0;
	...
	*/
	...
	for (int i = 0; i<M_num; i++)
	{
		sp_info = SPFA2(nodes,i, node_num,three_day_strings,M_num);
