...
int main()
{
	...
	*/
	...
	cout << "solve_integrated_model:" << endl;
	IloEnv env;
	try
	{
	...
	//	MP_solve.exportModel("model3_.lp");
		...
		env.out() << "groung_vars_Values= " << vals << endl;
...
vector<oper_rotation*>two_day_BFS(double**cost_matrix_, int M1, int M2, int num_, vector<string_>S,int M_num)          //BFS遍历所有可行路径
{
	cout << "source" << M1<< endl;
...
vector<oper_rotation*>BFS_add_string(int*base, vector<vector<vector<string_>>>type_strings, double aircraft_dual, vector<leg*>leg_list, int M_num) // type_strings是各种string,s1-s4
{
	...
	*/
	...
	for (int i = 0; i<M_num; i++)
	{
		for (int j = M_num + three_day_strings.size(); j<node_num; j++)
		{
...
vector<vector<leg*>>OD::BFS(vector<BFS_treenode*>nodes_set,pair<double, int> ** cost_time_matrix_,int source,int s_t,int num_,int max_time,vector<leg*>weekly_legs_)          //BFS遍历所有可行路径
{
	...
	*/
	...
	while (!queue_.empty())
	{
		...
		//cout << "ummmm" << u->leg_->index<<endl;
		//cout << "nnnnnnnnnnn" << u->children.size() << endl;
		for (int i = 0; i < u->children.size(); i++)
		{
			//cout << "i"<<u->children[i]->leg_->index<< endl;
...
vector<vector<leg*>>OD::find_all_path(vector<leg*>&leglist_)
{
	...
	cout << "jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj" << nodes_set.size();
