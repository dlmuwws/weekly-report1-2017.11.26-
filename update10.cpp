...
vector<leg*>leg::get_day_leglist(vector<vector<event_node*>>daily_nodes_, int airport_num, int day_,double**leg_fc_)
{
		...
		if (!is_base_(i, base_))           
		{
			for (int j = 0; j < daily_nodes_[i].size() - 1; j++)        
			{
				for (int k = 0; k < airport_num; k++)
				{
					...
					{
						day_leg_list.push_back(new leg(index, day_, i, daily_nodes_[i][j]->time + (day_ - 1) * 1440, k, daily_nodes_[k][j + 1]->time + (day_ - 1) * 1440 - Min_groundtime, 0, leg_vc, leg_fc_[i][k], index));
						index++;
					}
				}
			}
	...
	{
		day_leg_list.push_back(new leg(index, day_, 1, daily_nodes_[1][j]->time + (day_ - 1) * 1440, 3, daily_nodes_[3][j + 1]->time + (day_ - 1) * 1440 - Min_groundtime, 0, leg_vc, leg_fc_[1][3],index));
		index++;
	}
	for (int j = (daily_nodes_[0].size() - 1) / 2; j < daily_nodes_[0].size() - 1; j++)
	{
		day_leg_list.push_back(new leg(index, day_, 0, daily_nodes_[0][j]->time + (day_ - 1) * 1440, 1, daily_nodes_[1][j + 1]->time + (day_ - 1) * 1440 - Min_groundtime, 0, leg_vc, leg_fc_[0][1], index));
		index++;
	}

	for (int j = 0; j < (daily_nodes_[2].size() + 1) / 2; j++)
	{
		day_leg_list.push_back(new leg(index, day_, 2, daily_nodes_[1][j]->time + (day_ - 1) * 1440, 4, daily_nodes_[4][j + 1]->time + (day_ - 1) * 1440 - Min_groundtime, 0, leg_vc, leg_fc_[2][4], index));
		index++;
	}
	for (int j = (daily_nodes_[3].size() - 1) / 2; j < daily_nodes_[3].size() - 1; j++)
	{
		day_leg_list.push_back(new leg(index, day_, 3, daily_nodes_[3][j]->time + (day_ - 1) * 1440, 2, daily_nodes_[2][j + 1]->time + (day_ - 1) * 1440 - Min_groundtime, 0, leg_vc, leg_fc_[3][2], index));
		index++;
	}
