...
G_arc*leg::get_y1_a(vector<vector<G_arc*>>G)
{
	...
	{
		if (arrival_t == G[dest_airp][i]->end_->time)
		{
...
G_arc*leg::get_y2_a(vector<vector<G_arc*>>G)
{
	...
	for (int i = 0; i<G[dest_airp].size(); i++)
	{
		if (arrival_t == G[dest_airp][i]->start_->time)
		{
			return G[dest_airp][i];
		}
	}
...
double oper_path::cacu_mp(vector<OD*>OD_info)   //caculate margin profits
{
	...
	{
		var_costs += this->legs[i]->var_cost+this->legs[i]->capacity_dual;
	}
		...
		//	IloNumArray leg_cover_dual(env, daily_leg_list.size());
			...
			else
			{
				...
				for (int i = 0; i < string_list.size(); i++)
				{
					...
					{
						col += weight_cons[string_list[i]->legs[j]->index - 1](-max_weight);
					}
					for (int j = 0; j < string_list[i]->legs.size(); j++)
					{
						col += cover_cons[string_list[i]->legs[j]->index - 1](1);
					}
					col += balance_cons1[string_list[i]->legs[0]->index - 1](1);
					col += balance_cons2[string_list[i]->legs[string_list[i]->legs.size() - 1]->index - 1](-1);
