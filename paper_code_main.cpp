...
int main()
{
	...
	try
	{
		...
		IloNumVarArray flight_var(env);
		IloNumVarArray rotation_var(env);
		...
		for (int i = 0; i<leg_list.size(); i++)
		{
			...
			weight_cons.add(IloRange(env, 0, max_weight, name.c_str()));
		}
		...
		for (int i = 0; i<leg_list.size(); i++)
		{
			string name = string("leg") + string("volume_cons");
		...
		for (int i=0;i<flight_list.size();i++)
		{
			...
			string var_name = string("flight_var");
			flight_var.add(IloNumVar(col, 0, 1, ILOFLOAT, var_name.c_str()));
		}
		INT_model.add(flight_var);

		cout << "_" << flight_var.getSize() << endl;
		//rotation var
		for (int i=0;i<oper_rotation_list.size();i++)
		{
			...
			string var_name = string("rotation_var");
			rotation_var.add(IloNumVar(col, 0, 1, ILOFLOAT, var_name.c_str()));
		}
		INT_model.add(rotation_var);
		cout << "_" << rotation_var.getSize() << endl;
		...
		for (;;)
		{
			...
			for (int i = 0; i < OD_info.size(); i++)
			{
				...
				{
					leg_list[j]->set_var_cost(leg_list[j]->var_cost + OD_info[i]->vol_unit*v_price[j] + w_price[j]);
				}
				oper_path*path_ = new oper_path(i, OD_info[i]->find_shortest_path(leg_list));
