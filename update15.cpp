...
vector<oper_rotation*>add_string(int*base, vector<vector<vector<string_>>>type_strings, double aircraft_dual, vector<leg*>leg_list, int M_num, vector<vector<G_arc*>>daily_gc_, vector<leg*>weekly_leg_list_);
...
pair<bool, int*>SPFA1(double**cost_matrix_, int source, int num_);
pair<bool, int*>SPFA2(double**cost_matrix_, int source, int num_, vector<vector<string_>>three_day_strings, int M_num);
...
vector<oper_rotation*>two_day_find_sp(int*base, vector<vector<string_>>&two_day_strings_, int M_num, double aircraft_dual_, vector<leg*>&leg_list, vector<leg*>&week_leg_list, vector<vector<G_arc*>>daily_gc_);
vector<oper_rotation*>three_day_find_sp(int*base, vector<vector<string_>>three_day_strings, int M_num, double aircraft_dual_, vector<leg*>&leg_list, vector<leg*>&week_leg_list, vector<vector<G_arc*>>daily_gc_);
...
int main()
{
	...
	cout << "type1_string_size:" << type_strings_[0][0].size() << endl;   //2
	cout << "type2_string_size:" << type_strings_[1][0].size() << endl;   //6
	cout << "type3_string_size:" << type_strings_[2][0].size() << endl;    //6
	cout << "type4_string_size:" << type_strings_[3][0].size() << endl;    //3
	...
	*/
	...
	try
	{
		...
		cout << "initial_flow_vars_size:" << flow_vars.getSize() << endl;
		*/
		//string var
		...
		int*base_ = new int[base_num];
		base_[0] = 1; base_[1] = 2;
		string_list = add_string(base_, type_strings_, 0, daily_leg_list, base_num, d_G,weekly_leg_list);
		...
		cout << "initial_string_vars_size:" << string_vars.getSize() << endl;
		for (int i = 0; i<string_list.size(); i++)
		{
	...
	//	vector<double>rc_
		...
		for (;;)
		{
			...
			*/
			...
			to_add_strings = add_string(base_, type_strings_, aircraft_num_dual, daily_leg_list, base_num, d_G, weekly_leg_list);
			for (int i = 0; i<to_add_strings.size(); i++)
			{
...
vector<oper_rotation*>add_string(int*base, vector<vector<vector<string_>>>type_strings, double aircraft_dual, vector<leg*>leg_list, int M_num, vector<vector<G_arc*>>daily_gc_, vector<leg*>weekly_leg_list_) // type_strings是各种string,s1-s4
{
	...
	vector<oper_rotation*>two_day_find_sps_ = two_day_find_sp(base, two_day_strings, M_num, aircraft_dual, leg_list,weekly_leg_list_, daily_gc_);
	...
	vector<oper_rotation*>three_day_find_sps_ = three_day_find_sp(base, three_day_strings, M_num, aircraft_dual, leg_list,weekly_leg_list_, daily_gc_);
...
vector<oper_rotation*>two_day_find_sp(int*base, vector<vector<string_>>&two_day_strings_, int M_num, double aircraft_dual_, vector<leg*>&leg_list, vector<leg*>&week_leg_list,vector<vector<G_arc*>>daily_gc_)   //s2[0]和s4[1]  //找出从各个M出发的最短两天string
{
	...
	for (int i = 0; i < M_num; i++)
	{
		for (int j = M_num; j < M_num + size1; j++)
		{
			if (s2_[j - M_num].front()->leg_->origin_airp == base[i])
			{
				...
				edges[s].cost_=s2_[j - M_num].front()->leg_->balance1_dual + cacu_string_cost(s2_[j - M_num],week_leg_list) + (aircraft_fc + aircraft_dual_)*get_rs_2(s2_string_rs(s2_[j - M_num], daily_gc_));
	...
	for (int i = M_num; i < M_num + size1; i++)
	{
		for (int j = M_num + size1; j <M_num + size1 + size2; j++)
		{
			if (s2_[i - M_num].back()->leg_->dest_airp == s4_[j - M_num - size1].front()->leg_->origin_airp&&can_connect(s2_[i - M_num], s4_[j - M_num - size1]))
			{
				...
				//cout << "iiiiiiiiiiiiii" << cost_matrix[i][j] << endl;
				s++;
				cost_matrix[i][j] = cacu_string_cost(s4_[j - M_num - size1],week_leg_list) + (aircraft_fc + aircraft_dual_)*get_rs_2(s4_string_rs(s4_[ j - M_num - size1], daily_gc_));
	...
	for (int i = 0; i<M_num; i++)
	{
		sp_info = SPFA1(cost_matrix, i, node_num);
		if (sp_info.first)
		{
...
pair<bool, int*>SPFA1(double**cost_matrix_, int source,int num_)
{
	...
	while (!queue_.empty())
	{
		...
		for (int i=0;i<num_;i++)
		{
			if (cost_matrix_[u][i]<INF&&dis_[i]>dis_[u]+cost_matrix_[u][i])  //松弛
			{
				dis_[i] = dis_[u] + cost_matrix_[u][i];
				pre_[i] = u;
				if (!visit_[i])   //i不在队列
				{
					queue_.push(i);
					count[i]++;
					if (count[i] >= num_)  //存在负权环
					{
...
pair<bool, int*>SPFA2(double**cost_matrix_, int source, int num_, vector<vector<string_>>three_day_strings, int M_num)
{
	...
	while (!queue_.empty())
	{
		...
		if (u>=M_num+size1&&u<M_num+size1+size2)
		{
			for (int i = 0; i < num_; i++)
			{
				if (cost_matrix_[u][i]<INF&&dis_[i]>dis_[u] + cost_matrix_[u][i]&&can_connect(s2_[pre_[u]-M_num],s4_[i-M_num-size1-size2]))
				{
					dis_[i] = dis_[u] + cost_matrix_[u][i];
					pre_[i] = u;
					if (!visit_[i])
		...
		else
		{
			for (int i = 0; i < num_; i++)
			{
				if (cost_matrix_[u][i]<INF&&dis_[i]>dis_[u] + cost_matrix_[u][i])
				{
					dis_[i] = dis_[u] + cost_matrix_[u][i];
					pre_[i] = u;
					if (!visit_[i])
...
vector<oper_rotation*>three_day_find_sp(int*base, vector<vector<string_>>three_day_strings, int M_num, double aircraft_dual_, vector<leg*>&leg_list, vector<leg*>&week_leg_list,vector<vector<G_arc*>>daily_gc_)   //s2[0]和s4[1]  //找出从各个M出发的最短两天string
{
	int size1 = three_day_strings[0].size();
	int size2 = three_day_strings[1].size();
	int size3 = three_day_strings[2].size();
...
//	int t = 0;
	...
	*/
	...
	for (int i = 0; i<M_num; i++)
	{
		sp_info = SPFA2(cost_matrix, i, node_num,three_day_strings,M_num);
...
vector<vector<event_node*>>daily_nodes(int airport_num)
{
	...
	{
		nodes[i].push_back(new event_node(i, 360, NULL));
		int n = 0;
		for (int t = 9; t <= 24; t += 3)
		{
