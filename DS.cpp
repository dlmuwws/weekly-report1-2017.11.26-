...
template<typename Type> void RadixSort(DataList<int> &list, int m, int d) {
	LinkQueue<int> *queue = new LinkQueue<int>[d];//dynamic
	int pow= 1;
	for (int i = 0; i<m; i++) {
		if (i) {
			pow = pow * d;
		...
		for (int j = 0; j<list.m_ncurrentsize; j++) {
			int k = (list.m_pvector[j].GetKey() / pow) % d;
...
template<typename Type> void InsertSort(DataList<Type> &list, int n) {
	if (-1 == n) {
...
template<typename Type> void BinaryInsertSort(DataList<Type> &list, int n) {
	if (-1 == n) {
		for (int i = 1; i<list.m_ncurrentsize; i++) {
...
template<typename Type> void ShellSort(DataList<Type> &list, const int gap) {
	if (-1 == gap) {
...
template<typename Type> void BubbleSwap(DataList<Type> &list, const int n, int &flag) {
	flag = 0;
...
template<typename Type> void BubbleSort(DataList<Type> &list) {
	int flag = 1, n = 0;
...
template<typename Type> void QuickSort(DataList<Type> &list, int left = 0, int right = -1) {
	if (-3 == right) {
...
template<typename Type> void SelectChange(DataList<Type> &list, const int n) {
	int j = n;
...
template<typename Type> void SelectSort(DataList<Type> &list) {
	for (int i = 0; i<list.m_ncurrentsize - 1; i++) {
		SelectChange(list, i);
	}
}

//template<typename Type> void HeapAdj(DataList<Type> &list, const int start, const int end) {
...
template<typename Type> void HeapSort(DataList<Type> &list) {
	for (int i = (list.m_ncurrentsize - 2) / 2; i >= 0; i--) {
		HeapAdj(list, i, list.m_ncurrentsize - 1);
	}
...
template<typename Type> void MergeD(DataList<Type> &list, DataList<Type> &mergedlist, const int start, const int part, const int end) {
...
template<typename Type> void Merge(DataList<Type> &list, DataList<Type> &mergedlist, const int len) {
	int n = 0;
	...
	if (n + len < list.m_ncurrentsize) {
		MergeD(list, mergedlist, n, n + len - 1, list.m_ncurrentsize - 1);
	}
	else {
