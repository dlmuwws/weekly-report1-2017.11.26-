			...
			*/
			...
			if (q != to_add_strings.size())
			{
				...
				for (int i = 0; i <_add_strings.size(); i++)
				{
					...
					_itoa_s(i + 1, str, 10);
					_itoa_s(a, str1, 10);
					string var_name = string("----------")+ string(str1)+string("new_string_var_") + string(str);
				//	IloNumVar this_string(col, 0, 1, ILOFLOAT, var_name.c_str());
				//	IloNum my_rc = _add_strings[i]->cacu_RC(aircraft_num_dual, weekly_leg_list);
				//	IloNum cplex_rc = MP_solve.getReducedCost(this_string);
				//	if (my_rc!=cplex_rc)
				//	{
				//		cout << "-------------m" << my_rc << endl;
					//	cout << "=====" << cplex_rc << endl;
					...
					//IloNum my_rc = _add_strings[i]->cacu_RC(aircraft_num_dual, weekly_leg_list);
				//	cout << "ccc" << endl;
				///	IloNum cplex_rc = MP_solve.getReducedCost(string_vars[string_vars.getSize()-1]);
					//cout << "ffff" << endl;
					//if (my_rc != cplex_rc)
				///	{
					//	cout << "-------------m" << my_rc << endl;
	...
	for (int i = M_num; i < M_num + size1; i++)
	{
		for (int j = M_num + size1; j <M_num + size1 + size2; j++)
		{
			if (s2_[i - M_num].back()->leg_->dest_airp == s4_[j - M_num - size1].front()->leg_->origin_airp&&can_connect(s2_[i - M_num], s4_[j - M_num - size1]))
			{
				...
				cost_matrix[i][j] = cacu_string_cost(s4_[j - M_num - size1],week_leg_list) + (aircraft_fc + aircraft_dual_)*get_rs_2(s4_string_rs(s4_[ j - M_num - size1], daily_gc_)) - s4_[j - M_num - size1].back()->leg_->balance2_dual;
	...
	for (int i = M_num + size1; i < M_num + size1 + size2; i++)
	{
		for (int j = M_num + size1 + size2; j <M_num + size1 + size2 + M_num; j++)
		{
			...
			if (s4_[i - M_num - size1].back()->leg_->dest_airp == base[j - M_num - size1 - size2])
			{
				...
				cout << "ssssssss" << cost_matrix[i][j]<<endl;
			}
		}
	}
	*/
...
vector<oper_rotation*>three_day_find_sp(int*base, vector<vector<string_>>three_day_strings, int M_num, double aircraft_dual_, vector<leg*>&leg_list, vector<leg*>&week_leg_list,vector<vector<G_arc*>>daily_gc_)   //s2[0]和s4[1]  //找出从各个M出发的最短两天string
{
	...
	int node_num = M_num + size1 + size2 + size3;
	double ** cost_matrix = new double *[node_num];
	for (int i = 0; i < node_num; i++)
	{
		cost_matrix[i] = new double[node_num];
	...
	for (int i = M_num + size1; i < M_num + size1 + size2; i++)
	{
		for (int j = M_num + size1 + size2; j <M_num + size1 + size2 + size3; j++)
		{
			...
			{
				cost_matrix[i][j] = cacu_string_cost(s4_[j - M_num - size1 - size2],week_leg_list) + (aircraft_fc + aircraft_dual_)*get_rs_2(s4_string_rs(s4_[j - M_num - size1 - size2], daily_gc_)) - s4_[j - M_num - size1 - size2].back()->leg_->balance2_dual;
	...
	*/
	...
	for (int i = 0; i<three_strings.size(); i++)
	{
		...
		{
			for (int k = 0; k<three_strings[i][j].size(); k++)
			{
	...
	for (int i = 0; i<M_num; i++)
	{
		sp_info = string_Dijkstra2(i, cost_matrix, node_num,three_day_strings,M_num);
